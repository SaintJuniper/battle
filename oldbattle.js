...
let myCards = [
  { name: "A", a: 4, hp: 2 },
  { name: "B", a: 3, hp: 3, attr: { taunt: true } },
  { name: "C", a: 1, hp: 1 },
];

// This is the order of the enemy cards
let enemyCardsOrdered = [
  { a: 1, hp: 1 },
  { a: 2, hp: 2 },
  { a: 3, hp: 5 },
];
...
function resetBoard() {
  ...
  for (let i = 0; i < myCards.length; i++) {
    addToBoard(boardA, i, { a: myCards[i].a, hp: myCards[i].hp });
  }

  // Enemy board
  for (let i = 0; i < enemyCardsOrdered.length; i++) {
    addToBoard(boardB, i, {
      a: enemyCardsOrdered[i].a,
      hp: enemyCardsOrdered[i].hp,
    });
  }
}
...
function fight(attackerBoard, defenderBoard, attackerIndex, defenderIndex) {
  ...
  attackerAtk = attackerBoard[attackerIndex].a;
  defenderAtk = defenderBoard[defenderIndex].a;

  attackerHp = attackerBoard[attackerIndex].hp;
  defenderHp = defenderBoard[defenderIndex].hp;

  if (attackerAtk >= defenderHp) {
    removeFromBoard(defenderBoard, defenderIndex);
  } else {
    defenderBoard[defenderIndex].hp -= attackerAtk;
  }

  if (defenderAtk >= attackerHp) {
    removeFromBoard(attackerBoard, attackerIndex);
  } else {
    attackerBoard[attackerIndex].hp -= defenderAtk;
  }
}
...
function startCombat() {
  ...
  while (checkWin() > 1) {
    ...
    if (turn == true) {
      enemyTargetIndex = Math.floor(Math.random() * boardB.length);
      fight(boardA, boardB, initA, enemyTargetIndex);
      initA = (initA + 1) % boardA.length;
    } else {
      enemyTargetIndex = Math.floor(Math.random() * boardA.length);
